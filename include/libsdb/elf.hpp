...
#include <filesystem>
#include <libsdb/detail/object_format.hpp>
#include <memory>
#include <deque>
#include <vector>
#include <string_view>
...
namespace sdb {
    ...
    private:
        ...
        std::deque<std::string> section_names_, symbol_names_;

        std::uint64_t preferred_base_ = 0;
        void parse_macho(const std::filesystem::path&, bool debug_only);
    public:
        std::uint64_t preferred_base() const { return preferred_base_; }

        std::uint64_t offset_to_address(std::uint64_t offset) const;
    private:
        int fd_;
        ...
        std::unordered_multimap<std::string, Elf64_Sym*>
            symbol_name_map_;
