...
#include <utility>
namespace { constexpr std::pair<std::string_view,int> calls[]={
{"syscall",0},
{"exit",1},
{"fork",2},
{"read",3},
...
std::string_view sdb::syscall_id_to_name(int id){for(auto [n,i]:calls)if(i==id)return n;error::send("Unknown Darwin syscall");}
int sdb::syscall_name_to_id(std::string_view name){for(auto [n,i]:calls)if(n==name)return i;error::send("Unknown Darwin syscall");}
