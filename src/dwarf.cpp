...
namespace {
    ...
    public:
        ...
        cursor& operator+=(std::size_t size) { if(!pos_ || size > static_cast<std::size_t>(data_.end()-pos_)) sdb::error::send("Truncated DWARF data"); pos_ += size; return *this; }
        ...
        std::string_view string() {
            ...
            if(null_terminator == data_.end()) sdb::error::send("Unterminated DWARF string");
    ...
    std::uint64_t parse_eh_frame_pointer(
        ...
        case DW_EH_PE_pcrel:
            base = elf.offset_to_address(pc); break;
    ...
    std::unique_ptr<sdb::call_frame_information>
        ...
        auto eh_hdr = dwarf.elf_file()->get_section(".eh_frame_hdr") ? parse_eh_hdr(dwarf) : sdb::call_frame_information::eh_hdr{};
