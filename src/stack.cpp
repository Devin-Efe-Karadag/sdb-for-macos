...
void sdb::stack::reset_inline_height() {
	...
	for (auto it = stack.rbegin();
		it != stack.rend() and it->abbrev_entry()->tag == DW_TAG_inlined_subroutine and it->low_pc() == pc;
		++it) {
		++inline_height_;
	}
}
...
const sdb::registers& sdb::stack::regs() const {
	if (frames_.empty()) return target_->get_process().get_registers(tid_);
	return frames_.at(current_frame_).regs;
}

sdb::virt_addr sdb::stack::get_pc() const {
	return virt_addr{
		regs().read_by_id_as<std::uint64_t>(sdb::program_counter_register)
	};
}
...
void sdb::stack::unwind() {
	...
	while (virt_pc.addr() != 0 and elf and frames_.size() < 256) {
		...
		virt_pc = virt_addr{
			regs.read_by_id_as<std::uint64_t>(program_counter_register) - 1
		};
...
void sdb::stack::create_base_frame(
	...
	if (line_entry != line_table::iterator{}) frames_.back().location = source_location{
